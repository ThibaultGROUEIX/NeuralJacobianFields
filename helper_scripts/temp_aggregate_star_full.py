...
import sys
import shutil
files_to_copy = ['target.obj','handles.npy']
def main(s_dir = '.',t_dir = '<target_folder>',doit = False):
    counter =0
    for w in os.walk(s_dir):
        full_src = w[0]
        without_extra_slash = os.path.normpath(full_src)
        rel_src = os.path.basename(full_src)
        if not rel_src.isdigit():
            continue
        print(full_src)
        rel_src = f'{int(rel_src):08d}'
        for ind,c in enumerate(files_to_copy):
            copy_and_rename(c, full_src, rel_src, t_dir,ind==0,doit)
            if counter % 100 == 0:
